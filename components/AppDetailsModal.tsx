...
import { X, Youtube, BookOpen, User, Code, Tag, Activity, ArrowRight, Globe, Github, LifeBuoy, ChevronLeft, ChevronRight, ImageIcon } from 'lucide-react';
        ...
        <div className="flex-grow overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-6 items-start">
            ...
            <div className="md:col-span-2 space-y-8">
              {/* Smaller Screenshot Carousel displaying ~2.5 images */}
              {hasScreenshots && (
                <div className="relative group">
                  ...
                  >
                    {app.screenshots?.map((src, idx) => (
                      <div key={idx} className="flex-shrink-0 w-[85%] md:w-[260px] aspect-video bg-slate-800 rounded-lg overflow-hidden border border-slate-700 snap-start shadow-lg relative group/item">
                        <img 
                          src={src} 
                  ...
                  {/* Navigation Buttons - Adjusted position for smaller items */}
                  <button 
                    onClick={() => scroll('left')}
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {app.website && (
                    <a href={app.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 text-slate-300 border border-slate-700 rounded-md hover:bg-slate-700 transition-all text-xs font-medium">
                      <Globe className="w-3.5 h-3.5" /> Website
                    </a>
                  )}
                  {app.repo_link && (
                    <a href={app.repo_link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 text-slate-300 border border-slate-700 rounded-md hover:bg-slate-700 transition-all text-xs font-medium">
                      <Github className="w-3.5 h-3.5" /> Source
                    </a>
                  )}
                  {app.support_link && (
                    <a href={app.support_link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 text-slate-300 border border-slate-700 rounded-md hover:bg-slate-700 transition-all text-xs font-medium">
                      <LifeBuoy className="w-3.5 h-3.5" /> Support
                    </a>
                  )}
                  {app.youtube && (
                    <a href={app.youtube} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-1.5 bg-red-600/10 text-red-500 border border-red-500/20 rounded-md hover:bg-red-600 hover:text-white transition-all text-xs font-medium">
